...
"""
    ...
    # Fallback to api_key.json if env variables are not set
    if not base_url or not api_key:
        try:
            with open('../api_key.json', 'r', encoding='utf-8') as file:
                api_data = json.load(file)
                base_url = base_url or api_data[0].get("base_url")
                api_key = api_key or api_data[0].get("api_key")
        except FileNotFoundError:
            pass

    if not api_key:
        print("Error: OpenAI API Key not found in .env or api_key.json")
        return
